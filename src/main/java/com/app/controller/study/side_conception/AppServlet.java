...
public class AppServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response ) throws ServletException, IOException {
		...
		if(requestURI.equals("접속경로비교")) {
			...
			Enumeration<String> paramNames = request.getParameterNames();// 파라미터 이름(key)값들만 
			...
			//paraMap 전달
			MyModelAndView mav = con3.process(null);
			// viewName 옮겨 담기
			MyView myView = new MyView();
			//myView.viewName = mav.viewName; //mav에 저장된 viewName 활용
		...
		if(requestURI.equals("접속경로비교")) {
			...
			String viewName = con4.process(null, null); //model, paramMap
